    ...
    def _train_ppo(self):
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, N, self.batch_size):
                ...
                # BUGFIX_RATIO_CLAMP: prevent ratio explosion (was 43911!)
                ...
                # BUGFIX_KL_EARLYSTOP: detect catastrophic policy drift BEFORE update
                with torch.no_grad():
                    approx_kl = ((ratio - 1.0) - log_ratio).mean().item()
                if approx_kl > 0.075:  # 1.5x target_kl=0.05
                    if n_minibatches <= 2 or n_minibatches % 8 == 0:
                        print(f"[PPO] KL early-stop at mb#{n_minibatches}: "
                              f"approx_kl={approx_kl:.4f} > 0.075 (skip update)")
                    stats["kl"].append(float(approx_kl))
                    ...
                    break  # skip rest of this PPO-epoch
